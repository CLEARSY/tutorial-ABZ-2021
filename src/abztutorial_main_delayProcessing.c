...
void failure_delayProcessing(uint8_t p_announcementPedal, uint8_t p_freeingPedal);
...
void SECTION_NON_REPLICATED_NON_SHARED_FUNCTION api_main()
{
    ...
    while(1) {
		...
		failure_delayProcessing(l_announcementPedalState,l_freeingPedalState);
...
void SECTION_NON_REPLICATED_NON_SHARED_FUNCTION failure_delayProcessing(uint8_t p_announcementPedal, uint8_t p_freeingPedal)
{
	...
	if(l_date >= s_nextExecutionTime)
	{
		...
		r1__r_f__lc_treatement((uint32_t)(l_time >> 32),(uint32_t)(l_time & 0xffffffff),p_announcementPedal,p_freeingPedal);
        r2__r_f__lc_treatement((uint32_t)(l_time >> 32),(uint32_t)(l_time & 0xffffffff),p_announcementPedal,p_freeingPedal);
	}
